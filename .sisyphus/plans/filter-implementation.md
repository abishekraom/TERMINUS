...
- [x] Open `globe/frontend/src/components/ViewModeSelector.jsx`.
- [x] Remove the objects for `flir`, `anime`, and `snow` from the `FILTER_MODES` array.
- [x] Open `globe/frontend/src/components/AppLayout.jsx`.
- [x] Change `const [activeMode, setActiveMode] = useState('crt');` to `useState('normal');`.
...
- [x] Open `globe/frontend/src/components/AppLayout.jsx`.
- [x] Update the `<Header />` instantiation to pass the active mode: `<Header activeMode={activeMode} />`.
- [x] Open `globe/frontend/src/components/Header.jsx`.
- [x] Accept `activeMode` as a prop: `export default function Header({ activeMode = 'normal' })`.
- [x] Find the hardcoded `<span className="text-amber font-bold tracking-wider text-glow-amber">CRT</span>`.
- [x] Replace "CRT" with `{activeMode.toUpperCase()}`.
...
- [x] Open `globe/frontend/src/index.css`.
- [x] Add the new filter classes at the bottom:
  ```css
  ...
  ```
- [x] Open `globe/frontend/src/components/AppLayout.jsx`.
- [x] Add the dynamic class to the wrapper div holding the canvas (the second main div, class `"flex-1 relative overflow-hidden"`).
- [x] Modify the `scanlines-overlay` element to be conditional:
...
- [x] Review the entire app manually by running `npm run dev` in the `globe/frontend` folder and clicking through the filters. Verify no remnants of FLIR, ANIME, or SNOW exist in the UI or codebase.
