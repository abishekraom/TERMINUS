...
async function handleAisMessage(raw) {
  ...
  if (normalized._isStaticUpdate) {
    ...
    upsertVesselProfile(normalized.vesselId, normalized).catch(() => {});
    return;
  }
  ...
  const merged = {
    ...
    vesselType: (existing.vesselType && existing.vesselType !== 'UNKNOWN') ? existing.vesselType : normalized.vesselType,
  ...
  appendTrailPoint(merged.vesselId, merged.trail[merged.trail.length - 1]).catch(() => {});
}
...
async function main() {
  ...
  // 3. Start stale data reaper (120s TTL)
  const reaper = createReaper(vesselStore, removeVesselState, removeVesselFull, {
    staleMemoryMs: 300_000,
    staleFirebaseMs: 1_800_000,
  });
  const reaperInterval = reaper.start(120_000); // Check every 2 minutes
