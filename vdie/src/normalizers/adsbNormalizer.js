...
// category filter: strictly include 4 (large), 5 (high vortex large) as per PRD §4.1
const INCLUDE_CATEGORIES = new Set([3, 4, 5, 6]);      // small, large, high vortex large, heavy

function isCategoryAllowed(category) {
  if (category == null) return false;               // drop unknowns — strict cargo only
...
function resolveAdsbCategory(category) {
  if (category === 3) return 'AIRCRAFT_COMMERCIAL_REGIONAL';
  if (category === 4) return 'AIRCRAFT_PASSENGER';
  if (category === 5) return 'AIRCRAFT_PASSENGER'; // high vortex large
...
function normalizeStateVector(sv) {
  ...
  // Filter by category: strictly include 4, 5, 6 as per PRD §4.1
