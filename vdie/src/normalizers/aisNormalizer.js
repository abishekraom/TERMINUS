...
const SKIP_TYPES = new Set([35]); // Military ops only (Fishing kept for display)
...
function resolveVesselCategory(typeCode) {
  ...
  if (typeCode === 35) return 'UNKNOWN'; // Military ops
...
function isStale(timeUtc) {
  ...
  return ageMs > 120_000; // 120 seconds per spec
}
...
function normalizePositionReport(raw) {
  ...
  let dataQuality = msg.PositionAccuracy ? 'HIGH' : 'MEDIUM';
  // PRD Contradiction Check: claims underway but SOG is essentially 0
  if (sog < 0.5 && navStatus === 0) {
    dataQuality = 'LOW';
  }
  vessel.dataQuality = dataQuality;
